...
class Test(object):
    ...
    def predict(model, series):
        series = np.asarray(series, dtype=np.str)
        x_data = convert_data(series)
        dm_x_data = xgb.DMatrix(x_data)
        pred = model.predict(dm_x_data)
        out = pd.DataFrame({"Before": series, "Predicted": pred})
...
if __name__ == "__main__":
    ...
    df = Test.predict(model, ["On", "22th November 1999", "the" ,"fastest", "swimmer", "was", "25 km/h", ".", "It", "was", "22:59",".", "CO2", "is", "bad", "!", "22th", "November", "1999"])
   
    result = Test.convert(df)
